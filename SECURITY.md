...
MCP OAuth sessions use a 400-day idle and 730-day absolute ceiling to support unattended integrations. Five-minute access tokens, refresh-token rotation and replay detection remain enabled. Clients must protect and atomically rotate long-lived credentials; compromising a refresh token can grant ongoing access until revocation. Browser client limits and the local one-hour browser cookie remain shorter. See [session boundaries and monitoring](docs/OAUTH-SESSIONS.md).
