...
Mail connections require verified TLS 1.2 or later. STARTTLS must succeed before credentials are sent. Mail hostnames are operator-allowlisted; DNS is resolved and public addresses validated before connecting to a pinned IP with the original hostname for certificate verification. Arbitrary URL or local file loading through SMTP is disabled. Only authenticated users can send, using their own SMTP credentials.
