...
The realm policy supports 400 days idle and 730 days maximum for MCP authorization sessions, with five-minute access tokens and refresh rotation. See [OAuth session lifetimes, client renewal requirements and health checks](OAUTH-SESSIONS.md). Existing realms require the dedicated session migration; restarting the container does not apply realm-template changes.
